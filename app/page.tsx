...
export default function Home() {
  ...
  function move(direction: number) {
    const pair = revision ? (progress.pair + direction + pairs.length) % pairs.length : nextPair(progress, direction);
      ...
      <section className="practice-area" aria-label="Practice dashboard">
        ...
        {!authenticated ? <section className="welcome-card"><Leaf size={32}/><h2>A little practice. All yours.</h2><p>Sign in to keep your progress with you, wherever you practice.</p><button className="primary" onClick={() => setModal('login')}>Continue with Google <ArrowRight size={17}/></button><button className="text-button" onClick={() => setPreview(true)}>Try a local preview</button></section> : complete && !revision ? <section className="welcome-card"><CheckCheck size={36}/><div className="eyebrow">32 SMALL STEPS. REAL PROGRESS.</div><h2>You finished the collection.</h2><p>Make the patterns stick. Revisit a pair and explain your approach out loud.</p><button className="primary" onClick={() => setRevision(true)}>Start a revision round <RotateCcw size={16}/></button></section> : <article className="practice-card">
          ...
          <div className="problems">{current.map((problem, i) => { const done = progress.solved.includes(problem.id); return <div className={`problem ${done ? 'is-done' : ''}`} key={problem.id}>
            <span className="problem-number">0{i + 1}</span><div className="problem-main"><div className="tags"><span className={`difficulty ${problem.difficulty.toLowerCase()}`}>{problem.difficulty}</span><span className="tag-dot">·</span><span>{problem.topic}</span></div><h3>{problem.title}</h3><p>{problem.note}</p><div className="problem-links"><a className="solve-link" href={problem.url} target="_blank" rel="noopener noreferrer"><Code2 size={15}/> Open {problem.platform} <ArrowUpRight size={14}/></a><span className="estimate">~{problem.minutes} min</span></div></div>
          ...
          <div className="card-footer"><span className={pairDone ? 'pair-complete' : ''}>{pairDone ? <CheckCheck size={16}/> : <Leaf size={16}/>}<span>{pairDone ? 'Nice work. Another step forward.' : 'Take your time. Make it count.'}</span></span><div className="pair-navigation"><button className="previous" aria-label="Previous pair" disabled={!ready || busy} onClick={() => move(-1)}><ArrowLeft size={16}/></button><button className="primary" disabled={!ready || busy} onClick={() => move(1)}>Next pair <ArrowRight size={16}/></button></div></div>
        </article>}
